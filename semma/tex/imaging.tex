...
where f is the focal length, a is the distance between the lens and the film, and b is the distance between the lens and the imaged source.
...
{Barrel (left) and pincushion distortions that would show up when shooting a wall with straight lines. For a non-distorted lens, the lines would not be curved.}
...
Clapperboards are a ubiquitous and simple way to sync video and audio, but it still leaves a maximum of half a frame lag between the camera sequences; this is illustrated in figure \ref{fig:syncproblems}.
...
Video recording and motion tracking are best considered orhtogonal issues; while a single static case can be scanned in three dimensions, so can be also each frame of a sequence, separately.

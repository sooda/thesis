...
\simplegfx{p}{0.8\textwidth}{setupcams}{
...
VisualSFM \cite{visualsfm} was tested with dense reconstruction done by PMVS \cite{pmvs} and final cleanup and meshing in Meshlab \cite{meshlab}.
...
\includegraphics[width=0.3\textwidth]{arto-3}
	...
	Note the shadows under the jaw, the nose, and near the eyes; only the top flash was used for this dataset for demonstration.
}
...
\includegraphics[width=0.45\textwidth]{artoglassuvspherical}
	...
	Photoscan can employ a ``generic'' texture mapping mode with no distortion in the pictures, and a ``spherical'' mode where the subject is assumed to be a sphere and the texture is made continuous but the original photos are warped.
}
...
\includegraphics[width=0.9\textwidth]{soodaheadreconsteyeswire}
	...
	Note that the mesh does not yet employ the full geometric properties of the source photograph, i.e.\ the pores and wrinkles.
}
...
%}
%
Other subjects were also scanned, and their results can be seen below.
...
\includegraphics[height=0.22\textheight]{horsereconst}\\
	...
	Subjects listed: book, hand, plushie, head, statue, horse.
}
...
% shrinkwrap modifier in blender

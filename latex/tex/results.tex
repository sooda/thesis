...
This particular fan's speed had to be reduced by using 4.5 volts instead of the nominal 12 volts, resulting in about TODO revolutions per second, or TODO m/s on the far end of the fan blades.
...
\fbox{\includegraphics[width=0.3\textwidth]{flashtiming-c}}
	...
	In the first one, the flash has been fired before the frame exposure started, and the frame shows the flash shutting down;
	the frame before it is black without any overexposed light.
...
The results answer questions on what kinds of surfaces can be reconstructed well; different materials are given for the datasets.
...
The Canon EOS 700Ds have internal pop-up flash lights; they were not used, as they were found to trigger inconsistently, and they are too bright and small so that dark shadows and highlights become a problem.
...
White styrofoam boards were left to the sides of the rig to balance the lighting.
The general setup is shown in Figure \ref{fig:totalsetuptwo}, and closer depiction of the poses of the cameras and the flash unit bounced from downwards are shown in Figure \ref{fig:frontview}.
...
Similar guidelines are usually proposed, such as a camera baseline of 10 \% of the distance to the subject.
...
Agisoft PhotoScan is a self-contained pipeline, and VisualSFM does only part of the SFM reconstruction, leaving much work to external programs, providing flexibility in selecting the different algorithms.

PhotoScan is a popular commercial program for scanning subjects from several pictures for generation of 3D models and measuring spatial data.
...
VisualSFM \cite{visualsfm} performs dense reconstruction using PMVS \cite{pmvs} and final cleanup and meshing were performed manually in Meshlab \cite{meshlab} using the Poisson surface reconstruction plugin, all free software.
...
No installation instructions are provided in this thesis; refer to documentation of the presented programs for installation and usage.
...
A typical face scan with surrounding geometry removed results in a high-density point cloud of three million points, which the software reduces into a textured mesh with exactly five times less triangles than the original points.
The meshing process was found to consume temporarily a maximum of 4 GB of RAM.
...
The tested reconstructions in PhotoScan have significant noise all surfaces for unknown reasons, though.
...
Large-scale wrinkle detail tests can be seen in Figure \ref{fig:sample-arto-forehead}; the point cloud and the mesh have enough detail to encode forehead wrinkles in the geometry, in addition to the texture color.
...
\includegraphics[width=0.5\textwidth]{arto-forehead-tex}\\%
	...
	The darkened color in the wrinkles is directly encoded in the color texture.
}
\simplefig{h}{%
\includegraphics[width=0.45\textwidth]{artoglassuvgeneric}%
\includegraphics[width=0.45\textwidth]{artoglassuvspherical}
}{fig:sample-arto-photoscan-source}{
	PhotoScan provides a ``generic'' texture mapping mode with no distortion in the pictures, and a ``spherical'' mode where the subject is assumed to be a sphere and the texture is made continuous but the original photos are warped.
...
\paragraph{VisualSFM and Meshlab}
The work pipeline in VisualSFM is composed in parts that are done mostly by external programs; feature extraction and matching is done first, followed by SfM using libpba, and finally dense reconstruction is performed by PMVS/CMVS (CMVS always resulted in only a single cluster).
Final Poisson surface reconstruction is done by opening the resulting dense point cloud in Meshlab.
...
The results of a face scan are depicted in Figure \ref{fig:sample-sooda-meshlab}; the surface has less noise than that of 

Figure \ref{fig:soodaheadclosecompare} shows a cropped region of one of the source photographs in the \emph{head} dataset.
...
High-level detail could be encoded in the color texture, but since the detail is geometric, computer renderings using detailed light models would result in a more realistic rendering if the appearance would change as a function of light direction.
Beeler et al. propose a mesoscopic augmentation from the captured images to increase the geometric detail level, and point out that they are not reconstructed only because of a too small camera resolution. \cite{beeler2010high}
...
\includegraphics[width=0.5\textwidth]{meshlab-head-wired}
	...
	A dense point set, solid triangle mesh, wireframe rendering of the mesh, and the textured mesh in Meshlab.
...
%}
%
Other subjects were also scanned, and their results are summarized in Figure \ref{fig:vsfm-compare}.
...
Only problems in human scans are hair that is not well distinguishable and does not present any clear surface, and bright spots that result from a combination of bad lighting and sweating.
A horse head mask made of rubber has problems only in white and black areas that have no texture and either either provide incorrect or no points at all.
Surprisingly, a moderately textured soft plushie provided well reconstructed geometry even in less textured areas, where the fabric provides enough detail to distinguish any correspondences.
A painted statue presented difficulties in some shiny areas, but otherwise, the whole geometry was found to be correct even in visually weakly textured areas.

A simple lego structure provided to work surprisingly well, even though the lego blocks have a plain color without much texture; the only texture comes from the corners of different colored blocks.
A textured shirt was tested and found to work with a fewer number of views, as the shirt is very planar and subsequent views do not bring much new information.
A 
...
The corners between blocks of different color are the only well reconstructed 
...
in this experiment, the lights were particularly overly bright and the subject used sunglasses for eye protection; less than three of the lights resulted in visible shadows in parts of the face.
...
The video was shot with a larger baseline which VisualSFM could not cope with; thus, results could be analyzed only in PhotoScan.
Incorrect scene structure resulted in some frames with high-speed motion in the mouth while the subject was talking.
...
(FIXME rename section to Results and add Discussion as a subsection?)

The presented examples show that the system is feasible for capturing many kinds of objects; even better quality would probably be achieved with tuning algorithm parameters manually for each subject.
...
The camera array synchronization and rolling shutter compensation methods proposed by Bradley et al.\ \cite{bradley2009synchronization} should be experimented on the system, as both these issues are present in the current setup.
...
% join "acquiring the reflectance" to the geometry capture for relighting \cite{debevec2000acquiring}

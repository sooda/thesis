...
In addition to multiple cameras at different locations and poses to encode a subject's geometry, an uniform and soft lightning is also required to minimize bright reflection difficulties in the texture.
...
Powerful flash light sources use usually a gas-filled tube that is excited with high voltage to produce a short, bright burst of light.
...
More discussed in the section \ref{sec:video}.
Shutter duration that exposes half of the frame time is common in professional motion pictures \cite{wilson2004anton}.

...
% TODO this is the first actual paragraph and needs to be much better

%Stereo vision works by analyzing two or more images for depth cues.
% TODO in addition too hollsten's number of points, note also the actual accuracy of the model
...
\simplegfx{t}{\textwidth}{tinyflowergrid}{
	...
	Each pixel has a specific color shown as a red-green-blue triplet; thus, the image is completely defined by $23 * 34 * 3 = 2346$ numbers.
}
...
\simplegfx{t}{0.6\textwidth}{pinhole-camera}{
	...
	Image formed through the small hole is formed to the plane on the back of the box, rotated 180 degrees, as rays of light travel from the subject through the hole.
}
...
%\simplegfx{t}{0.6\textwidth}{coordsys}
...
When a light ray diverges on a small aperture due to the wavelike nature of light, the different distances traveled make the rays interfere each other and produce a two-dimensional diffraction pattern, called an \emph{airy disk} \cite{greenleaf1950photographic}.
...
The disk size can be approximated \cite{greenleaf1950photographic} by
\begin{equation} \label{eq:airydisk}
	c = 1.22 \frac{\lambda f}{d} = 1.22 \lambda N
\end{equation}
which, for example, for red light in the visible spectrum having a wavelength $\lambda$ of 700 nanometers, with an aperture of f/11, results in $\approx$ 0.01 mm (compare to above CoC of 0.03 mm).
...
% FIXME centi-pixel etc. villiers \cite{villiers2008centi}
\simplefig{t}{%
...
\includegraphics[width=0.2\textwidth]{pincushion-distortion}
	...
	(FIXME placeholder, replaced by real-world images) % yes, building/trees? would bend but not related to close range. speaker? ruler? legos? something measurable
}
...
The radially symmetric radial error model and correction proposed first by Brown \cite{brown1971close,brown1966decentering} and used directly or with slight alternations in programming libraries, e.g., OpenCV, \cite{opencv} is % to create a new image of the original pixel values at new positions is
\begin{align} \label{eq:radialdist} \begin{split}
	x_{corr} &= x(1 + \alpha_1 r^2 + \alpha_2 r^4 + \alpha_3 r^6)\\
	y_{corr} &= y(1 + \alpha_1 r^2 + \alpha_2 r^4 + \alpha_3 r^6).
\end{split} \end{align}
where $x$ and $y$ are the original coordinates in the distorted image on the film or image sensor, $x_{corr}$ and $y_{corr}$ are the corrected ones, $\alpha_1$, $\alpha_2$, $\alpha_3$ are coefficients specific to the radial distortion and $r$ equals to the distance to optical origin, located at $(x_c,~y_c)$:
\begin{equation}
r = \sqrt{(x - x_c)^2 + (y - y_c)^2}
\end{equation}

Trucco and Verri \cite{trucco1998introductory} present only the two first coefficients, and it is not rare to use only the first $\alpha_1$; for lenses with low distortion, this is often enough.
...
x_{corr} &= x + 2 \beta_1 x y + \beta_2 (r^2 + 2 x^2)\\
y_{corr} &= y + 2 \beta_2 x y + \beta_1 (r^2 + 2 y^2)
\end{split} \end{align}
where $x$, $y$ $x_{corr}$, $y_{corr}$ and $r$ are as in Equation \ref{eq:radialdist} and $\beta_1$ and $\beta_2$ are new coefficients specific to tangential distortion.

The two independent types of distortion are combined by summing both to the original coordinate, combining Equations \ref{eq:radialdist} and \ref{eq:tangdist}:

\begin{align} \label{eq:bothdist} \begin{split}
	x_{corr} &= x (1 + \alpha_1 r^2 + \alpha_2 r^4 + \alpha_3 r^6) + 2 \beta_1 x y + \beta_2 (r^2 + 2 x^2)\\
	y_{corr} &= y (1 + \alpha_1 r^2 + \alpha_2 r^4 + \alpha_3 r^6) + 2 \beta_2 x y + \beta_1 (r^2 + 2 y^2)
\end{split} \end{align}
...
\simplegfx{t}{\textwidth}{raddist}{
	...
	The arrows point from original coordinates $(x, y)$ to corrected ones $(x_{corr}, y_{corr})$; the inverse would be necessary for undistortion.
	From left to right: $\alpha_1 = 0.16$, $\alpha_1 = -0.16$, $\alpha_2 = -0.08$, $\alpha_1 = -0.04, \alpha_2 = -0.04$.
...
The Brown's model describes the distortion characteristics for a lens system on an otherwise perfect image, but typically the inverse transform is required, because a distorted image is given as noted by Villiers et~al.\ \cite{villiers2008centi}. % and implemented in OpenCV \cite{opencv}.
...
\simplegfx{p}{0.99\textwidth}{focalplanerolling}{
	...
	Fast movement during the exposure leads to artifacts and a rapid flash light would illuminate only a part of the film.
}
...
% XXX unify refs and describe shot noise better
% XXX specify sensitivity and use different terms for different types of sensitivity
...
As all electronic circuits, also image sensors are subject to signal noise. % bad language
...
Such distribution model of the surface's global or local microgeometry specifies how much light reflects in different directions \cite{nayar1991surface}. % XXX 1991 or -89? % XXX N different sources, clarify...
...
%In reality, a number of other factors also have a small effect.
...
In contrast, in motion picture industry the shutter is kept open deliberately so long that fast motion is blurred, because it is considered aesthetically pleasing to human eye;
...
%For unsynchronized videos that shoot frames at different points in time, morphing techniques have been developed to analyze motion between frames and to interpolate intelligently. \cite{bradley2009synchronization}
...
This can be compensated to some degree with stroboscopic lighting or morphing between recorded frames if hard syncing is not possible. \cite{bradley2009synchronization}
...
%For example, according to Equation \ref{eq:gigabit-transfer}, one network interface per each camera would be needed for a decent video rate.
...
\begin{itemize}
	\item Camera Link, partly proprietary serial transfer protocol with special cable and connector types % (but significantly reverse-engineered)
...
\simplegfx{b}{0.7\textwidth}{reconstpipeline}{
	...
	FIXME: draft image depicting the idea
}
...
The \emph{extrinsic parameters} contain the camera location and rotation in another global coordinate system, often structured as a matrix \cite[p. 41]{hartley03multiview}. % , \cite{heyden2005multiple}
...
% no prior calibration known

Given pixel coordinates $\vec a_l$ and $\vec a_r$ that match the points $\vec A_l$ and $\vec A_r$ as seen from cameras $\vec C_l$ and $\vec C_r$, respectively, the fundamental matrix $\bm F$ is given as:
...
Another popular method is \emph{blob detection}, based on the laplacian of a Gaussian-filtered image \cite{?}.
Edge and blob detection methods work as a basis for several feature matching and detection algorithms. % XXX cite, describe blob and edge detection

% XXX some sift descriptor theory, angle histogram stuff
...
%The dense correspondence search is typically not done with feature matching but comparing pixels directly instead.
...
\simplegfx{t}{0.5\textwidth}{rectification}{
	...
	Image by Pollefeys \cite{pollefeys2004visual}.
}
...
% XXX osittaisderivaatta
\simplegfx{t}{\textwidth}{baselineerr}{
	The effect of baseline difference $||A-B||$ on the depth error $Z_2 - Z_1$, with a small (left) and large (right) baseline.
...
% XXX again a bit bad intro paragraph
...
%Similarly, videos can be slowed down such that the resulting motion appears smoother by estimating a number of intermediate frames \cite{eugster2011slowmovideo}.
% The Matrix.

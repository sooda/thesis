...
The aperture size is presented as relative to the focal length and is commonly called the \emph{F number} \cite[p.~23]{greenleaf1950photographic}, specified via the aperture diameter $d$:
...
Full multiples of $\sqrt 2$ are called \emph{stops}, and the equivalent f number changes as a function of subject distance \cite[p.~23]{greenleaf1950photographic}, which is left out here for simplicity.
...
Acceptable size of the CoC depends on the resolving power of the viewer; when a photograph is looked at, the print size and distance of the image determines the circle \cite[p.~26]{greenleaf1950photographic}.
...
%Hox ALL-I (intra) / IPB (intra-predict-bidirpredict)
...
%where $T$ is the cross-product form of $t$ encoded in a matrix form as below.
%The essential matrix is obtained as $E = R T$.

...
when using a standard cartesian coordinate system, a point is described as a location relative to a selected origin reference, as a triplet of distances along three perpendicular X, Y and Z axes. % (figure \ref{fig:coordsys}).
...
As the blur happens gradually, a \emph{circle of confusion} is often used as a definition of unsharp areas of images.
...
Depth of field can be calculated from the difference between the shortest and farthest object distances that generates a circle of confusion, found geometrically.
When $f$ is the focal length, $F$ the aperture f number, $s$ the distance to the subject, and $c$ the acceptable circle of confusion, the depth of field $D$ is given by (\cite{greenleaf1950photographic}):
...
Size of the circle of confusion depends on the resolution power of the viewer; when a photograph is looked with the eye, the print size of the image determines the circle.
A popular formula in photography determines the size as $c = d/1730$, computed via the diagonal $d$ of the image size.
...
\simplegfx{h!}{\textwidth}{dofcoc}{
	Exaggerated illustration of circle of confusion and depth of field, seen from the side (not drawn to scale).
	...
	It can be easily seen that at longer distances, the CoC becomes smaller.
}
...
%\simplefig{h!}{\input{raddist.tex}}{fig:raddist}
...
Fast shutter movement is depicted in figure \ref{fig:focalplanerolling}.
...
The reconstruction industry uses the term 4D in the context of time-varying 3D data.

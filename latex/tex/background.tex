...
%For best results, it is important that the source images for stereo vision algorithms are in focus and error-free \cite{hollsten2013imagequality}.
...
\simplegfx{t}{\textwidth}{camerapipeline}{
...
% XXX full-frame camera sensor pixel size
...
The radial error model and correction proposed first by Brown \cite{brown1971close,brown1966decentering} and used directly or with slight alternations in programming libraries, e.g., OpenCV, \cite{opencv} is % to create a new image of the original pixel values at new positions is
...
\simplegfx{t}{\textwidth}{raddist}{
	Barrel-type radial distortion with different values of two parameters.
	Lines point from non-distorted image pixels to distorted ones; the inverse would be necessary for undistortion.
	Left: $K_1 = 0.16$, middle: $K_2 = 0.08$, right: $K_1 = -0.04, K_2 = -0.04$.
	The different shapes of second and fourth order polynomials are easily visible, and the error is most effective in the borders of the image.
}
...
Because digital images consist of a discrete pixel array, the pixel coordinates given by the undistortion do not necessarily lie on exact pixel positions; the sampling should then interpolate between neighboring pixels \cite{wolberg1990digital}.
...
\includegraphics[width=0.7\textwidth]{pinhole3d}
	...
	Camera coordinate system origin and the physical pinhole at O, axis X3 points towards the optical axis, Y1 and Y2 point to image plane axes and R is the principal point, at the image center.
...
As seen in Figure \ref{fig:epigeom}, a 3D point A seen by the left camera $C_l$ could be anywhere on the line between $C_l$'s origin and A, because a certain line passing through the principal point always projects to a point (the camera position is equal to the principal point, i.e., the pinhole)
This line is seen as a single point $A_l$.
From another viewpoint in the right camera $C_r$, this line equals to some line on the right image plane.
...
\begin{equation} \label{eq:essential}
	A_l E A_r = 0
\end{equation}
% XXX  by the corresponding camera coordinates --- AA_l . T x AA_r
...
% blah blah these coords ($X_l$, $X_r$) are in camera coordinate frames but from the images only pixel values can be measured
...
% assdfasdoh estimating the fundamental matrix from a set of matching points in two images, the epipolar geometry and thus the 3D position of the points are recovered.
...
%Now, given its position in the image plane of camera $C_l$ and the fundamental matrix $F$, finding the corresponding point in camera $C_r$'s image plane is seen as solving Equation \ref{eq:fundamental} for $a_r$.
...
Edge and blob detection methods work as a basis for several feature matching and detection algorithms. % CITE
...
\simplegfx{t}{0.5\textwidth}{rectification}{
	...
	The common plane is parallel to the baseline vector $P_k-P_l$.
